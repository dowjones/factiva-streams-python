...
class Config(object):
    ...
    DEFAULT_CUST_CONFIG_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), './stream_config.json'))
    ...
    def get_authentication_headers(self):
        ...
        if user_key:
            ...
            msg = '''Could not find determine credentials:
                Must specify account credentials as user_id, client_id, and password, either through env vars,
                customer_config.json, or as args to Listener constructor (see README.rst)'''
            raise Exception(msg)
    ...
    # return credentials (user_id, client_id, and password) for obtaining a JWT via OAuth2 if all these fields
    # are defined in the constructor, env vars or config file otherwise return None (the client will have to
    # authenticate Extraction API request with an user key, i.e. the old way)
