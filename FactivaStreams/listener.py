...
from FactivaStreams.config import Config
from FactivaStreams.services import pubsub_service, credentials_service
            ...
            try:
                ...
                if results:
                    if len(results.received_messages) > 0:
                        with open(os.path.join(ARTICLES_PATH, "articles_{}.json".format(int(time.time()))), "wt", encoding="utf-8") as articles_file:
                            for message in results.received_messages:
                                pubsub_msg = json.loads(message.message.data)
                                logger.debug("Received news message with ID: {}".format(pubsub_msg['data'][0]['id']))
                                news_msg = pubsub_msg['data'][0]['attributes']
                                callback_result = on_message_callback(news_msg, subscription_id, articles_file)
